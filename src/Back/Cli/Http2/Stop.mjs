/**
 * Stop HTTP/2 server.
 * @namespace TeqFw_Web_Back_Cli_Http2_Stop
 */
// MODULE'S IMPORT
...
// DEFINE WORKING VARS
const NS = 'TeqFw_Web_Back_Cli_Http2_Stop';
 ...
 * @constructor
 * @memberOf TeqFw_Web_Back_Cli_Http2_Stop
 */
function Factory(spec) {
    ...
    const action = async function () {
        try {
            ...
            console.info(`Stop HTTP/2 server (PID: ${pid}).`);
            process.kill(pid, 'SIGINT');
        } catch (e) {
            console.error('Cannot kill HTTP/2 server process.');
        }
    };
